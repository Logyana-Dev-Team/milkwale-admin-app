...
const AddDelboyModal = (props) => {
  ...
  const fetchData = async () => {
    ...
    if (responseData.delboyS) {
      dispatch({
        type: "fetchDelboyAndChangeState",
        payload: responseData.delboyS,
      });
    }
  };
  ...
  const submitForm = async (e) => {
    ...
    //   return setFdata({ ...fData, error: "Please upload a Delboy image" });
    // }

    try {
      let responseData = await createDelboy(fData);
  ...
  return (
    <Fragment>
      {/* Black Overlay */}
      <div
        onClick={(e) => dispatch({ type: "addDelboyModal", payload: false })}
        className={`${
          data.addDelboyModal ? "" : "hidden"
        ...
        <div className="relative bg-white w-12/12 md:w-3/6 shadow-lg flex flex-col items-center space-y-4  overflow-y-auto px-4 py-4 md:px-8">
          <div className="flex items-center justify-between w-full pt-4">
            ...
            <span
              ...
              onClick={(e) =>
                dispatch({ type: "addDelboyModal", payload: false })
              }
          ...
          <form className="w-full" onSubmit={(e) => submitForm(e)}>
              ...
              <input
                onChange={(e) =>
                  setFdata({
                    ...
                    success: false,
                    error: false,
                    delphone: e.target.value,
                  })
                }
                value={fData.delphone}
                className="px-4 py-2 border focus:outline-none"
                type="number"
              />
            </div>
            ...
            {/* <div className="flex flex-col space-y-1 w-full">
              <label htmlFor="description">Delboy Description</label>
              <textarea
                onChange={(e) =>
                  setFdata({
                    ...fData,
            ...
            {/* <div className="flex flex-col space-y-1 w-full">
              <label htmlFor="name">Delboy Image</label>
              <input
                accept=".jpg, .jpeg, .png"
                onChange={(e) => {
                  setFdata({
            ...
            {/* <div className="flex flex-col space-y-1 w-full">
              <label htmlFor="status">Delboy Status</label>
              <select
                name="status"
                onChange={(e) =>
                  setFdata({
